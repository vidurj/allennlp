...
class SimpleSeq2Seq(Model):
    ...
    def forward(self,  # type: ignore
        ...
        for timestep in range(num_decoding_steps):
            if target_tokens is None:
                ...
                for batch_index in range(batch_size):
                    ...
                    if gold_token == '@@PADDING@@':
                        sampled_incorrect_predictions.append(int(targets_numpy[batch_index, timestep]))
                    else:
                        ...
                        assert gold_index != pred, (gold_index, pred)
                ...
                targets = Variable(torch.cuda.LongTensor(targets_numpy))
            ...
            corrupted_token_mask = torch.ones_like(class_probabilities)
            corrupted_token_mask[:, corrupted_token_index] = 0
            _, predicted_classes = torch.max(corrupted_token_mask * class_probabilities, 1)
