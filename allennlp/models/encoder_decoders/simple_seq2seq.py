...
class SimpleSeq2Seq(Model):
    ...
    def forward(self, **kwargs) -> Dict[str, torch.Tensor]:
        ...
        all_predictions = []
        batch_size = 1
        final_decoder_hidden = torch.cuda.FloatTensor(self._encoder_num_layers * 2, batch_size, self._encoder_output_dim).fill_(0)
        final_decoder_context = torch.cuda.FloatTensor(self._encoder_num_layers * 2, batch_size, self._encoder_output_dim).fill_(0)
        ...
        for sentence_number in range(len(sentence_number_to_text_field)):
            ...
            print('sentence length', len(list(source_tokens.values())[0]))
            source_mask = get_text_field_mask(source_tokens)
            embedded_input = self._source_embedder(source_tokens)
            batch_size, _, _ = embedded_input.size()
            ...
            all_predictions.extend(step_probabilities)
