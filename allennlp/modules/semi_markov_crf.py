...
class SemiMarkovConditionalRandomField(torch.nn.Module):
    ...
    def _input_likelihood(self,
        ...
        if tag_mask is None:
            tag_mask = torch.ones(batch_size, num_tags, device=device)
        else:
        ...
        default_tag_mask = torch.zeros(num_tags, device=device)
        default_tag_mask[self.default_tag] = float("-inf")
        default_tag_mask = default_tag_mask.view(1, 1, -1)
        ...
        alpha = torch.zeros(1, batch_size, device=device)
        ...
        for j in range(sequence_length):
            ...
            idx = torch.tensor([i for i in range(j, j - width, -1)], device=device)
            ...
            start_indices = torch.tensor(range(width), device=device)
    ...
    def _joint_likelihood(self,
        ...
        default_tags = self.default_tag * torch.ones(batch_size).long().to(logits.device)
    ...
    def forward(self,
        ...
        if self.loss_type == "roc":
            ...
            zeros = torch.zeros_like(inputs).to(inputs.device)
            cost = zeros
        else:
    ...
    def viterbi_tags(self,
                     logits: torch.Tensor,
                     mask: torch.Tensor,
                     tag_masks: torch.Tensor= None) -> List[List[int]]:
        """
        ...
        if tag_masks is None:
            tag_masks = torch.ones(batch_size, num_classes, device=device)
        else:
            tag_masks = tag_masks.to(device)
    ...
    def viterbi_decode(self, logits: torch.Tensor, tag_mask: torch.Tensor):
        ...
        for j in range(sequence_length):
            ...
            broadcast_alpha = torch.tensor(
                extended_alpha[j + 1 - width:j + 1][::-1], device=device)
        ...
        viterbi_score = torch.FloatTensor(
                sequence_length, max_span_width, num_classes,
                device=device).fill_(float("-inf"))
        viterbi_score[:, :, self.default_tag] = 0.0
        viterbi_score = viterbi_score.tolist()
    ...
    def _get_hamming_cost(self, tags: torch.Tensor) -> torch.Tensor:
        ...
        zeros = torch.zeros(batch_size, sequence_length,
                                     max_span_width, self.num_tags, device=tags.device)
    ...
    def _get_recall_oriented_cost(self, tags: torch.Tensor):
        ...
        fp = fp.index_fill_(-1,
                            torch.LongTensor([self.outside_span_tag], device=tags.device), 0)
